...
from tools.logging import get_logger, get_world_size_and_rank
...
class WandBLogger(MetricLoggerInterface):
    ...
    def log_table(
        self,
        name: str,
        table: Union["wandb.Table", pd.DataFrame, None] = None,
        *,
        ...
        """
        Log a W&B table so it shows up in the **Media → Tables** section.

        Parameters
        ----------
        name      Key under which the table appears in the UI.
        table     Pre-built ``wandb.Table`` **or** a pandas ``DataFrame``.
        columns   Column names (used only if ``table`` is *not* supplied).
        data      Row data      (used only if ``table`` is *not* supplied).
        step      Global step for proper x-axis alignment.
        """
        if not self._wandb.run:
            return  # no active run on non-rank-0 processes

        # Build a Table if the caller gave raw data
        if table is None:
            if columns is None or data is None:
                raise ValueError("Provide either `table` *or* (`columns`, `data`).")
            table = self._wandb.Table(columns=list(columns), data=list(data))
        elif isinstance(table, pd.DataFrame):
            table = self._wandb.Table(dataframe=table)
